...
class TweetsInPhases(Tweets):
    ...
    def _split_tweets_at_breakpoints(self) -> List[Tweets]:
        ...
        breakpoints_wrapped = [self.tweets['hour'].min().to_pydatetime()] + \
                              breakpoints + [self.tweets['hour'].max().to_pydatetime()]

        return [self.select_tweets_in_time_range(breakpoints_wrapped[i], breakpoints_wrapped[i + 1], 'hour')
