...
def pie_plot_multiplot(
    list_of_dfs: Sequence[pd.DataFrame], attributes_to_plot: Sequence
) -> go:
    """
    ...
    fig = make_subplots(
        rows=nr_rows_required,
        cols=NR_COLS,
        specs=specs_required,
    )

    for phase_i, phase_df in enumerate(list_of_dfs):
        for plot_counter, attribute in enumerate(
            attributes_to_plot, start=phase_i * len(attributes_to_plot)
        ):
            col = (plot_counter % NR_COLS) + 1
            row = (plot_counter // NR_COLS) + 1
            fig.add_trace(
                go.Pie(
                    labels=phase_df[attribute],
                    title=f"Phase {phase_i} analyzing {attribute}",
                ),
                row=row,
                col=col,
            )
