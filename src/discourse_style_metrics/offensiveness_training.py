...
ArgsDesc = namedtuple(
    "ArgsDesc",
    "dest num_epochs gradient_accumulation_steps batch_size learning_rate adam_epsilon only_prediction",
)
ARGS = ArgsDesc(
...
def read_germeval_data(
    file_paths: List[str], class_list: List[str], label_column: str = "task1"
) -> pd.DataFrame:
    ...
    training_df["label"] = training_df.apply(
        lambda x: class_list.index(x[label_column]), axis=1
    )
...
def main(model_name: str, train_files: List[str]):
    ...
    print(
        f"Train data has {len(X_train)} vals with {len(y_train)} labels;\n Test data has {len(X_test)}"
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, batch in enumerate(t):
            ...
            if (i + 1) % args.gradient_accumulation_steps == 0:
                ...
                optimizer.zero_grad()
            acc = (
                (output.logits.argmax(axis=-1).detach() == batch["label"])
                .float()
                .mean()
            )
            t.set_description(
                f"Epoch {epoch}, iter {i}, loss: {round(loss.item(), 4)}, acc: {round(acc.item(), 4)}"
            )
