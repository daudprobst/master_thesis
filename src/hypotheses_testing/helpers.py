...
def load_hypothesis_dataset(
    ...
    for name, query_dict in QUERIES.items():
        ...
        firestorm_tweets_query_set = (
            Tweets.objects(Q(search_params__query=query) & Q(lang="de"))
            .only(*attributes)
            .to_json()
        )

        firestorm_df = pd.DataFrame.from_records(loads(firestorm_tweets_query_set))
